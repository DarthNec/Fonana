...
## 📌 Регламент согласования решений перед фиксом выявленных проблем

[Регламент согласования решений перед фиксом выявленных проблем]

Цель

Исключить ситуации, когда выявленные в коде проблемы исправляются без предварительного обсуждения и согласования решений. Это позволит использовать уже накопленные знания и опыт команды и избегать лишних или некорректных исправлений.

Обязательный порядок действий при выявлении проблем
  1.  [Выявление проблемы]:
  • Обнаружив проблему, зафиксируй её чётко и кратко.
  • Не приступай к исправлению сразу же.
  2.  [Озвучивание проблемы]:
  • Сформулируй проблему максимально ясно и конкретно.
  • Укажи, какие последствия она может вызвать.
  3.  [Предложение возможных направлений решений]:
  • Предложи несколько возможных путей решения проблемы.
  • Коротко объясни плюсы и минусы каждого из вариантов.
  4.  [Согласование решений с командой]:
  • Получи обратную связь от команды (обсуждение, дополнения, выбор приоритетного решения).
  • После обсуждения команда утверждает один из предложенных вариантов.
  5.  [Планирование и исполнение]:
  • Только после согласования плана приступай к реализации.
  • Выполняй фикс строго по утвержденному плану.

Критерии успеха регламента

✅ Проблемы фиксируются только после предварительного обсуждения и согласования
✅ Используются и учитываются накопленные знания команды
✅ Сокращено количество ненужных или некорректных фиксов

Напоминания
  • Никогда не приступай к исправлениям без предварительного согласования с командой.
  • Фиксируй чётко выявленные проблемы и предлагаемые решения.
  • Документируй принятые решения и итоги обсуждений для дальнейшего использования.

# AI Chat Instructions for Fonana Project

## 🚨 КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА ДЕПЛОЯ (ОБЯЗАТЕЛЬНО!)

### 🚨 ПОЛНЫЙ ЧЕК-ЛИСТ ДЕПЛОЯ:

#### 1. **ПЕРЕД деплоем (ОБЯЗАТЕЛЬНО!):**
...
### 🚨 КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА РАБОТЫ С ФАЙЛАМИ:

#### 1. **Статические файлы (public/):**
...
#### 2. **ПОСЛЕ каждого деплоя (ОБЯЗАТЕЛЬНО!):**
   - ✅ **ОБЯЗАТЕЛЬНО** проверить, что изменения попали на сервер
   - ✅ **ОБЯЗАТЕЛЬНО** перезапустить PM2
   - ✅ **ОБЯЗАТЕЛЬНО** проверить результат в браузере
   - ✅ **ОБЯЗАТЕЛЬНО** проверить, что приложение работает корректно

### 🚨 ПОЛНЫЙ ЧЕК-ЛИСТ ПОСЛЕ ДЕПЛОЯ:
...
### 🚨 КРИТИЧЕСКИ ВАЖНЫЕ КОМАНДЫ ДЛЯ ПРОВЕРКИ:
...
- `public/` - Статические файлы (изображения, JS, CSS)
...
- **Deployment:** PM2 на Ubuntu сервере
...
./deploy-to-production.sh  # Деплой на продакшн
```
...
- **Posts** - Посты контент-мейкеров
- **Subscriptions** - Подписки на контент
- **Likes** - Лайки постов
- **Comments** - Комментарии к постам

### Wallet Integration
- Поддержка Phantom, Solflare, Backpack
- Автоматическое подключение при загрузке
- Персистентность подключения в localStorage

### Мобильная оптимизация
- Адаптивный дизайн для всех устройств
- PWA функциональность
- Service Worker для кеширования
- Оптимизированная навигация (4 кнопки на мобильных)
...
3. Перезапуск PM2 процесса
4. Проверка работоспособности

### Отладка
- Логи PM2: `pm2 logs fonana`
- Статус процессов: `pm2 status`
- Перезапуск: `pm2 restart fonana`
