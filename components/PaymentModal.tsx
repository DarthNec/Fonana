...
export function PaymentModal({
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const result = await executePayment(paymentParams, sendTransaction, publicKey)
      
      if (result.success) {
        toast.success(`Платёж успешно отправлен! Подпись: ${result.signature?.slice(0, 8)}...`)
        onClose()
      } else {
  ...
  const getPaymentTypeLabel = () => {
    switch (type) {
      ...
      case 'content': return 'Покупка контента'
              ...
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 p-6 text-left align-middle shadow-xl transition-all">
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-600 pt-4">
                    ...
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-gray-600 dark:text-gray-300">Комиссия платформы:</span>
                                             <span className="font-medium text-gray-900 dark:text-white">
                  ...
                  <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                    <p className="text-xs text-yellow-800 dark:text-yellow-300">
                      ...
                      Средства будут отправлены напрямую на кошелёк автора. Транзакция необратима.
                    </p>
                  </div>
                  ...
                  <div className="flex gap-3 pt-4">
                    ...
                    >
                      {isProcessing ? 'Обработка...' : `Отправить ${currency}`}
                    </button>
                  </div>
                </div>
              </Dialog.Panel>
